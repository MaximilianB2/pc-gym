...
def create_base_env_params(model_name):
    ...
    model_specific_params = {
        ...
        "biofilm_reactor": {
            ...
            "SP": {"S2_A": [12.5] * 100},
            "x0": np.array([10.0, 5.0, 5.0, 300.0] * 4 + [10]),
        },
        "crystallization": {
...
def test_oracle_with_custom_mpc_params(model_name):
    ...
    assert np.array_equal(oracle_instance.R, np.eye(env.Nu - env.Nd_model) * 3), (
        f"Custom control penalty not set correctly for {model_name}"
    )
    assert np.array_equal(oracle_instance.Q, np.eye(env.Nx_oracle) * 3), (
        f"Custom control penalty not set correctly for {model_name}"
    )
    x_log, u_log = oracle_instance.mpc()
...
def test_oracle_disturbance_performance(model_name):
    ...
    if model_name == "cstr":
        disturbance = {
            "Ti": np.repeat(
                [350, 345, 350],
                [base_params["N"] // 4, base_params["N"] // 2, base_params["N"] // 4],
            )
        }
