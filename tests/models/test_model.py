...
def test_constraints(model_name):
    ...
    params.update(
        {
            "constraints": lambda x, u: np.array([x[0] - 0.8, 0.9 - x[0]]).reshape(
                -1,
            ),
...
def test_disturbances(model_name):
    ...
    for _ in range(10):
        ...
        assert not np.allclose(
            state[-len(disturbances) :], next_state[-len(disturbances) :]
        )  # Disturbances should change
...
def test_JAX_int(model_name):
    ...
    params.update({"integration_method": "jax"})
